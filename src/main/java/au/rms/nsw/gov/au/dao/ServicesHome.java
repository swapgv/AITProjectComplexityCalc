 ...
 * @see au.rms.nsw.gov.au.dao.Services
...
public class ServicesHome {
	...
	public Services findById(java.math.BigDecimal id) {
		log.debug("getting Services instance with id: " + id);
		try {
			Services instance = (Services) sessionFactory.getCurrentSession().get("au.rms.nsw.gov.au.dao.Services", id);
			if (instance == null) {
	...
	public List<Services> findByExample(Services instance) {
		...
		try {
			List<Services> results = (List<Services>) sessionFactory.getCurrentSession()
					.createCriteria("au.rms.nsw.gov.au.dao.Services").add(create(instance)).list();
